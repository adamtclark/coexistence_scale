...
colnames(matout_tot)<-c("scale", "tscale", "iter",
                        ...
                        paste("eig_dist1", 1:nsp, sep="."),
                        paste("eig_dist2", 1:nsp, sep="."),
                        paste("r0_dist", 1:nsp, sep="."),
                        paste("eig_neut1", 1:nsp, sep="."),
                        paste("eig_neut2", 1:nsp, sep="."),
                        paste("r0_neut", 1:nsp, sep="."),
                        paste(paste(c("lag", "CV"), rep(c("meta", "dist", "neutral"), each=2), sep="."), rep(1:nsp, each=6), sep="."))
...
spmat<-rep(1:9, each=length(iterlst)*nsp)
spmat_lag<-rep(1:3, each=length(iterlst)*nsp)
...
#by species
matout_eigr0<-array(dim=c(length(scalslst), length(tscalelst), length(qtl_lims), nsp*9))
matout_invar<-array(dim=c(length(scalslst), length(laglst), length(qtl_lims), nsp*3))
matout_eigr0_noscl<-matout_eigr0

#means
matout_eigr0_mean<-array(dim=c(length(scalslst), length(tscalelst), length(qtl_lims),9))
matout_invar_mean<-array(dim=c(length(scalslst), length(laglst), length(qtl_lims), 3))
...
# By Species
###############
pdf("figures/stability_by_scale_iter.pdf", width=6, height=5, colormodel = "cmyk")
#pdf("figures/stability_by_scale_iter_local.pdf", width=6, height=4, colormodel = "cmyk")
par(mfrow=c(3,nsp), mar=c(2,2,1,1), oma=c(3,4,2,0))
...
for(adpos in tmp) {
  ...
  if(m==1) {
    ...
    mtext("Disturbance Model", 2, line=2.5)
  }else if(m==(2*nsp+1)) {
...
posmat<-matrix(ncol=3, data=c(1,4,7,10,2,5,8,11,3,6,9,12))
m<-1
for(adpos in 1:length(posmat)) {
  plot(range(laglst[lgldim]), range(c(0, matout_invar[sclsuse,lgldim,c(3), posmat[adpos]]), na.rm=T), type="n", xlab="time lag", ylab=expression(italic(paste("CV"))), xaxs="i",
       ylim=c(0, 0.35))
  abline(h=0, lty=3)
  ...
  n<-1
  for(scl in sclsuse) {
    matlines(laglst[lgldim], matout_invar[scl,lgldim,c(3),posmat[adpos]], col=collst[n], lty=1, lwd=lwduse)
    n<-n+1
  }
  box()
