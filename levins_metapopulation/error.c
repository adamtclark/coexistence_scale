...
static char *message[] =
{
  ...
  "F391%s  Warning: The array has not been filled",

  "F501%s  This feature is not yet supported",
  "F509%s  The pipe cannot be opened",
  ...
  "F519%s  A file I/O array index is out of range",
  "F520%s  The file I/O transformation does not begin with 'x' or 'n'",
  "F521%s  The file I/O transformation would divide by zero",
  "F522%s  The file I/O transformation is syntactically incorrect",
  "F523%s  A file I/O column heading is incorrect",
  "F524%s  A file I/O index label is incorrect [must be 'a'-'z']",
  "E525%s  The parameter is incorrect",
  "F526%s  A file I/O dimension is too large",
  "F527%s  A requested column label does not appear in the file",
  "F530%s  The file I/O structure must have all indices first",
  "F531%s  The file I/O structure must have at least one data column",
  "F532%s  The file has too many columns",
  "F533%s  A file I/O field contains spurious characters",
  "F534%s  A file I/O index field is incorrect",
  "F535%s  A file I/O field is too large",
  "F536%s  The file ended prematurely",
  "F537%s  The path identifier is out of range",
  "F538%s  No file name has been supplied",

  "E608%s  A parameter is out of range",
  "E609%s  The state is not correct",
  "E610%s  The individual number is incorrect",
  ...
  "E737%s  A new event would be scheduled too far in the future",
  "E738%s  A new event would be scheduled in the past",
  "E739%s  No tentative events are available for scheduling",
  ...
  "E760%s  A grouping operation cannot continue",
  ...
  "F851%s  The width 'w' of a clock's noise is not '0<=w<1'",
  "F852%s  Two conflicting distributions are specified for a clock",
...
indicate what is wrong. These parameters arrive in string-number pairs. One of
the routines 'Error', 'Error1', 'Error2', or 'Error3' is called, depending on
...
2. If the string ends with a special marker ('`'), both the string and the
number are both displayed as in 1 above, except the special marker itself is not
displayed.

3. If the string is null, the number is displayed.

4. If the string does not match 1 or 2 above, only the string is displayed, the
number is bypassed.
...
For example, here are some sample calls and the message they produce.
  ...
  Error1(511., "",1.618);             F501  The file cannot be read (1.618).
...
EXIT:  The message has been issued.  If the message number on entry was greater
         than or equal to 500, the routine never returns, but calls a routine
...
static void msgout(int r, char *s, char *pc[NPAR], char pn[NPAR][LNUM])
{
  ...
  for(i=0; i<NPAR && pc[i]; i++)             //If a text string is not marked,
    if(dropnum(pc[i])) pn[i][0] = 0;         //eliminate the number following.
  ...
  { for(j=0; pc[i][j]; j++)                  //were supplied.
      if(pc[i][j]!='`')
        fprintf(stderr,"%c", pc[i][j]);
...
EXIT:   'dropnum' is set if the string does not end with a selected character
          ('<', "=", '>', ':', or '`') or is not the null string,
*/

static int dropnum(char *s)
{ int c;

  if(s==0)    return 1;                      //Check for null strings.
  if(s[0]==0) return 0;

  c = s[strlen(s)-1];                        //If the string ends with one of
  if(c=='<'||c=='='||c=='>'||c==':'||c=='`') //the continuation symbols, allow
...
Routine 'ErrorInit' should be called at the beginning of processing to capture
errors such as invalid pointers. 'ErrorTrace' is called internally to
...
static void ErrBus() { Error(996.); }
static void ErrSeg() { Error(997.); }

int ErrorInit()
{ static int init;
  if(init) return 0;

  signal(SIGBUS,  ErrBus);                   //Capture bus errors.
  signal(SIGSEGV, ErrSeg);                   //Capture segmentation errors.
