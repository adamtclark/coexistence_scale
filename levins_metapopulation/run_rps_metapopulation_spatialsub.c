...
void run_rps_metapopulation_spatialsub(double *ptmax, int *pgridsize, int *pnsp, int xylim[], int destroyed[], int spdestroy[], //grid
		...
		int compmat[]) { //interaction matrix
	...
	int compeff = 0; //indicator of competitive forcing
	...
	while((tnow<tmax) && (totabund>0)) {
	 ...
	 if(eventtype==1) { //death event
	   ...
	   //look up competitive hierarchy
	   if((speciesid[newpos+cadj]!=nsp) && (speciesid[newpos+cadj]!=speciesid[sppos+cadj])) { // not empty, not same species
		//effect of column on row:
		//we want effect of resident species (newpos) on invading species (sppos)
		//row is speciesid[sppos+cadj] (resident species)
		//column is speciesid[newpos+cadj] (invading species)
		compeff=compmat[speciesid[sppos+cadj] + nsp*(speciesid[newpos+cadj])+cadj];

		//fprintf(stderr, "inv = %d ", speciesid[sppos+cadj]);
		//fprintf(stderr, "res = %d ", speciesid[newpos+cadj]);
		//fprintf(stderr, "int = %d \n", compeff);
	   } else { //empty, or same species
	   	compeff = 0;
	   }
	   ...
	   if((destroyed[newpos+cadj]==1) && (spdestroy[speciesid[sppos+cadj]]==1)) {
	     trigger=1;
	   } else if(compeff==1) { ////superior competitor
	     trigger=1;
	   } else if(speciesid[newpos+cadj]!=nsp) { ////inferior competitor
	     ...
	     speciesid[newpos+cadj]=nsp;
	   } // no else if speciesid==nsp: The cell is empty, trigger=0;
