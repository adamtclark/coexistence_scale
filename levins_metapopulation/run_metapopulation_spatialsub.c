...
void run_metapopulation_spatialsub(double *ptmax, int *pgridsize, int *pnsp, int xylim[], int destroyed[], int spdestroy[], //grid
	...
	while((tnow<tmax) && (totabund>0)) {
	 ...
	 if(eventtype==1) { //death event
	   ...
	   if((destroyed[newpos+cadj]==1) && (spdestroy[speciesid[sppos+cadj]]==1)) {
	     trigger=1;
	   } else if(speciesid[newpos+cadj]<=speciesid[sppos+cadj]) { ////check for superior competitor (or same species)
	     trigger=1;
	   } else if(speciesid[newpos+cadj]!=nsp) { ////check for inferior competitor
