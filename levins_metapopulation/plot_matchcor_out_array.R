...
if(length(grep("save_processed_data_FULL_array_2.RData", dir("output")))==0) {
  ...
  for(k in 1:length(scalslst)) {
    for(l in 1:length(tscallst_small)) {
      ...
      tmp<-matout_r0_pop[k,tscallst_small[l],,]
      tmp[!is.finite(tmp)]<-0
      dnsout_r0_pop<-apply(tmp, 1, function(x) try(density(x,na.rm=T), silent=T))
...
svg("figures/SUP_FIGURE_match_models_full.svg", width=5.5, height=8)
...
mtext(text = expression(paste("temporal extent, time steps")), side = 1, outer = TRUE, line = 1.3, cex=1.2, adj = 0.3+0.06)
