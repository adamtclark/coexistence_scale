...
#set up for runs
niterations<-100   #CHANGE TO ALTER NUMBER OF ITERATIONS
...
if(FALSE) {
  for(i in 1:niterations) {
    ...
    for(j in 1:length(scalelst)) {
      ...
      tmp<-apply(out2$output_spatial[,-1], 2, function(x) sd(x)/mean(x))
  ...
  save(list = c("varmat1", "varmat2"), file = "output/demstochvar.rda")
} else {
  load("output/demstochvar.rda")
}
...
pdf("figures/SUP_FIGURE_demstoch_vs_scale.pdf", colormodel = "cmyk", width=5, height=4)
  ...
  axis(1, las=2)
  axis(2, las=2)
  axis(3, las=2, at=scalelst*(100^2), labels = scalelst)
  mtext("spatial extent, fraction of maximum", 3, line=3)
  box()
  abline(v=scalelst*(100^2), lty=3)
dev.off()  
